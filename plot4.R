...
plot4 <- function(){
  ...
  with(airquality, {
    ...
    plot(dfPower$Time, dfPower$Sub_metering_1, type="l", xlab="", ylab="Energy sub metering")
    lines(dfPower$Time, dfPower$Sub_metering_2, col="red")
    lines(dfPower$Time, dfPower$Sub_metering_3, col="blue")
    legend("topright", col=c("black", "red", "blue"), legend=c("Sub_metering_1","Sub_metering_2", "Sub_metering_3"), lwd=c(2,2,2),bty = "n")
    plot(dfPower$Time, dfPower$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")
  })
